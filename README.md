...
然后在 HBuilderX 里选择这个 `scan` 文件夹作为本地原生插件即可。

## GitHub Actions

推送到 `master` 后会自动执行 `.github/workflows/build-ios-framework.yml`：

- 生成 `ios/ZhaiwoNativeScan.framework`
- 上传 `ZhaiwoNativeScan-framework` artifact
- 打包并上传 `Zhaiwo-NativeScan-plugin.zip`，可直接作为 HBuilderX 本地原生插件包下载
